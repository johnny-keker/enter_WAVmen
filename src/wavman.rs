...
pub fn generate_ambient(sample_rate: u32, num_samples: u32, bpm: u32) -> std::io::Result<Vec<u8>> {
  ...
  for _beat in 0..(num_beats) {
      start = generate_beat(&mut key, durations, part, sample_rate, seconds_per_beat, &mut buf, start);
  }
...
/*
    ...
    ("Y")   @KaluginaMarina
*/
fn write_part(bufer: &mut Vec<u8>, part: u32, note: f32, chord: notes_data::Chord, y: f32, sample_rate: u32, start: u32) -> u32 {
...
fn generate_beat(key: &mut notes_tools::UserKey, durations: [f32; 4], part: u32, sample_rate: u32, seconds_per_beat: u32, buf: &mut Vec<u8>, mut start: u32) -> u32 {
      ...
      while beat_remain != 0.0 {
          ...
          for _step in 0..(duration_in_parts / part) as u32 {
              start = write_part(buf, part, curr_note, curr_chord, y, sample_rate, start);
          }
